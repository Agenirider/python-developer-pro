...
class HTTPResponseMaker(object):
    ...
    def __call__(self):
        ...
        if self.GET:
            if url_status:
                logger("Request GET -  %s, Response 200 OK" % self.URL, "info")
                ...
                logger("Request GET -  %s, Response 404 Not Found" % self.URL, "error")
                return header_maker("404 Not Found")

        elif self.HEAD:
            if url_status:
                logger("Request HEAD, Response 200 OK", "info")
                return header_maker(
            ...
            logger("Request POST, Response 403 Forbidden", "error")
            return header_maker("403 Forbidden")

        else:
            logger("Request OTHER, 405 Method Not Allowed", "error")
            return header_maker("405 Method Not Allowed")
