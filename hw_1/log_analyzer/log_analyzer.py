...
def log_finder(configurations, logger = None):
    ...
    if register_date == current_date:
        logger.error( f"The log for {current_date} already performed") if logger is not None else None
        return None, None
        ...
        try:
            ...
            try:
                ...
                logs = [re.match(pattern, x).string \
                        for x in files if re.match(pattern, x) is not None][0]

                logger.info( f"Log file is {logs}")  if logger is not None else None

                return logs, current_date

            except IndexError:
                #  No log files
                logger.error( "No log files for %s" % current_date) if logger is not None else None
                return None, None
...
def tmp_cleaner(configurations, logger = None):
    ...
    if len(tmp_file) > 0:
        for file in tmp_file:
            ...
            os.remove(full_path)
            logger.info( f'File {file} deleted from TMP folder') if logger is not None else None
    ...
    for row in rows:
        ...
        try:
            ...
            try:
                ...
                logger.error( f"Unexpected row`s format {row}") if logger is not None else None

        except IndexError:
            count_err += 1
            logger.error( f"Unexpected row`s format {row if len(row) > 0 else 'EMPTY ROW'}") if logger is not None else None

    logger.info( f"Parsing result: ordinary rows - {count_all},"
                   f" unexpected format - {count_err},"
                   f" percentage {round((count_err / count_all) * 100, 3)}") if logger is not None else None
...
def log_performer(source, logger = None):
    ...
    for parsed_log_string in source:
        try:
            ...
            logger.error( f"Unexpected row`s data {parsed_log_string} ") if logger is not None else None
...
def register_writer(result_status, logger = None):
    ...
    if result_status is not False:
        try:
            ...
            logger.error( f'NOT FOUND REGISTER FILE FOR WRITING')  if logger is not None else None
...
def register_reader(logger = None):
    ...
    try:
        ...
        logger.error( f'NOT FOUND REGISTER FILE FOR READING')  if logger is not None else None
...
def main():
    ...
    try:
        if file is not None:
            ...
            register_writer((file_name_data if not bool(config.getboolean("DEFAULT", "DEBUG")) else False), logger)
